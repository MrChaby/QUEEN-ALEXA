...
const { getMessage } = require("./alexaplugins/sql/greetings");
const axios = require('axios');
const got = require('got');

// ════════════════════SQL◽◽◽◽
const QUEENALEXADB = config.DATABASE.define('QUEENALEXA', {
    info: {
...
async function QUEENALEXA () {
    await config.DATABASE.sync();
    var StrSes_Db = await QUEENALEXADB.findAll({
        where: {
          info: 'StringSession'
        }
    });
...
// ════════════════════WA CONNECTION◽◽◽◽    
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await QUEENALEXADB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
